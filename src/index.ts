...
function response(input: ObjColor): number {
    let suma: ObjColor = {
        r: 0,
        g: 0,
        b: 0,
    ...
    if (boolCompa.r) { suma.r = 1; }
    if (boolCompa.g) { suma.g = 1; }
    if (boolCompa.b) { suma.b = 1; }
...
function compa(val1: number, val2: number, key: string): number {
    let num = 0;
    console.log(val1, val2);

    if ((Math.round(val1)*10000)/10000 === (Math.round(val2)*10000)/10000){
        num = 1;
        if (key == 'r') { boolCompa.r = true; }
        if (key == 'g') { boolCompa.g = true; }
        if (key == 'b') { boolCompa.b = true; }
    }
